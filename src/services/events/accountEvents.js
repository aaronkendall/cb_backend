...
const accountEvents = (contract) => {
  contract.Creation(async (error, tx) => {
    ...
    try {
      ...
      const creationEvent = await new Event({  _creator: owner, type: 'Creation', message: `Fighter #${fighterId} created!` }).save()
  ...
  contract.Transfer(async (error, tx) => {
    ...
    try {
      ...
      const fromEvent = await new Event({ _creator: from, type: 'Transfer', message: `Fighter #${idNumber} transferred to ${to}` }).save()
      const toEvent = await new Event({ _creator: to, type: 'Transfer', message: `Fighter #${idNumber} transferred to you from ${from}` }).save()
  ...
  contract.AttributeIncrease(async (error, tx) => {
    ...
    try {
      ...
      const newEvent = await new Event({ _creator: owner, type: 'Attribute Increase', message: `Fighter #${fighterId}'s ${attribute} increased by ${increaseValue}!` }).save()
  ...
  contract.Healed(async (error, tx) => {
    ...
    try {
      const newEvent = await new Event({ _creator: owner, type: 'Heal', message: `Fighter #${fighterId} was healed to full health!` }).save()
      await Promise.all([
  ...
  contract.FighterFound(async (error, tx) => {
    ...
    try {
      const newEvent = await new Event({ _creator: owner, type: 'Fighter Found', message: `You found Fighter #${fighterId} while patrolling the streets!` }).save()
      await User.update({ address: owner }, { $addToSet: { events: newEvent } })
